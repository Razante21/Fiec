...
export default function PoloVeredas() {
  const modulos: ModuloData[] = [{ id: 'int-1', tag: 'Módulo Intermediário', dias: 'Quarta-feira', horario: '08h30 às 10h30', formUrl: 'LINK_FORM_VEREDAS_INT_4' }]
  ...
  return (
    <main className="min-h-screen">
      <motion.header initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ background: 'radial-gradient(ellipse 60% 60% at 8% 20%, rgba(245,166,35,.10) 0%, transparent 60%), linear-gradient(175deg, #091520 0%, #0d1a26 100%)', padding: '32px 24px 44px', textAlign: 'center', position: 'relative', borderBottom: '1px solid rgba(255,255,255,.07)' }}>
        ...
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 900, fontSize: 'clamp(2.2rem, 7vw, 3.8rem)', textTransform: 'uppercase', margin: 0 }}>Polo Veredas</motion.h1>
      </motion.header>
