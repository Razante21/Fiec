    ...
    <main className="min-h-screen">
      ...
      >
        ...
        <motion.div
          ...
          style={{ display: 'inline-block', background: '#f5a623', color: '#0d1a26', fontSize: '0.62rem', fontWeight: 800, padding: '3px 12px', borderRadius: '3px', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '12px' }}
        >
          Acima de 60 anos
        </motion.div>
        ...
        <motion.h1
          ...
          style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 900, fontSize: 'clamp(2.2rem, 7vw, 3.8rem)', textTransform: 'uppercase', margin: 0 }}
        >
          Polo Bem Viver
        </motion.h1>
      ...
      <div style={{ maxWidth: '1040px', margin: '44px auto', padding: '0 24px', display: 'grid', gridTemplateColumns: '270px 1fr', gap: '32px', alignItems: 'start' }}>
        <aside style={{ position: 'sticky', top: '24px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} style={{ background: '#1e3a52', borderRadius: '12px', border: '1px solid rgba(255,255,255,.07)', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '18px' }}>
              <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '1.4rem', fontWeight: 700, textTransform: 'uppercase', marginBottom: '6px' }}>Bem Viver</h2>
              <p style={{ color: '#8fb3cc', fontSize: '0.85rem' }}>Av. Eng. Fábio Roberto Barnabé, 585<br />Vila Teller</p>
            </div>
          </motion.div>
        ...
        <motion.main initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
          <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 900, fontSize: 'clamp(1.8rem,5vw,2.6rem)', textTransform: 'uppercase', marginBottom: '24px', paddingBottom: '14px', borderBottom: '1px solid rgba(255,255,255,.07)' }}>
          ...
          <motion.div variants={containerVariants} initial="hidden" animate="visible">
            <div style={{ marginBottom: '28px' }}>
              <motion.div variants={itemVariants} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                <h4 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '1rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#8fb3cc', whiteSpace: 'nowrap' }}>Básico / Celular</h4>
                <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,.07)' }} />
              </motion.div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '10px' }}>
                <ModuloCard modulo={modulos[0]} index={0} />
              </div>
            </div>

            <div style={{ marginBottom: '28px' }}>
              <motion.div variants={itemVariants} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                <h4 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '1rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#8fb3cc', whiteSpace: 'nowrap' }}>Intermediário / Celular</h4>
                <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,.07)' }} />
              </motion.div>

              <motion.div variants={itemVariants} style={{ background: 'rgba(61,186,126,.06)', border: '1px solid rgba(61,186,126,.18)', borderLeft: '3px solid #3dba7e', padding: '14px 16px', borderRadius: '8px', fontSize: '0.82rem', lineHeight: 1.6, color: '#8fb3cc', marginBottom: '18px' }}>
                <strong style={{ color: '#3dba7e' }}>Pré-requisito para esta turma:</strong><br />
                É necessário ter concluído o módulo básico do curso de inclusão digital da FIEC ou ter conhecimentos básicos de informática e computação.
              </motion.div>

              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '10px' }}>
                <ModuloCard modulo={modulos[1]} index={1} />
              </div>
            </div>
          </motion.div>
...
function ModuloCard({ modulo, index }: { modulo: ModuloData; index: number }) {
  return (
    <motion.a href={modulo.formUrl} target="_blank" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * index }} whileHover={{ y: -3, borderColor: '#f5a623', boxShadow: '0 8px 24px rgba(0,0,0,.3)' }} style={{ display: 'flex', flexDirection: 'column', padding: '16px 16px 14px', background: '#1e3a52', border: '1px solid rgba(255,255,255,.07)', borderRadius: '10px', textDecoration: 'none', color: 'white' }}>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '0.72rem', fontWeight: 700, fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '0.1em', textTransform: 'uppercase', color: '#4a9eca', marginTop: 'auto' }}>
        Inscrever-se <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><line x1="5" y1="12" x2="19" y2="12" /><polyline points="13 6 19 12 13 18" /></svg>
      </span>
    </motion.a>
  )
}