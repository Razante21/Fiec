...
function CreateTurma() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/users/turma', { nome, senha, turma, descricao, professor });
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-[#E8EFF7]" style={{ fontFamily: "'Syne',sans-serif" }}>Criar Turma</h1>
      </div>
      ...
      <div className="bg-[#111827] border border-[#1F2C42] rounded-2xl p-8">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs uppercase tracking-widest text-[#8DA4BF] mb-1.5">Nome da turma</label>
            <input type="text" value={turma} onChange={(e) => setTurma(e.target.value)} placeholder="Ex: Turma A — 2026" className="field-input" required />
          </div>

          <div>
            <label className="block text-xs uppercase tracking-widest text-[#8DA4BF] mb-1.5">Descrição</label>
            <input type="text" value={descricao} onChange={(e) => setDescricao(e.target.value)} placeholder="Breve descrição da turma" className="field-input" />
          </div>

          <div>
            <label className="block text-xs uppercase tracking-widest text-[#8DA4BF] mb-1.5">Professor responsável</label>
            <select value={professor} onChange={(e) => setProfessor(e.target.value)} className="field-input" required>
