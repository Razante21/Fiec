...
function CadastrarAula() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.titulo || !formData.descricao || !formData.data || !formData.tipo || !formData.ordem) {
    ...
    setLoading(true);
    try {
      await api.post('/aulas', { ...formData, turma: id });
      setSuccess('Aula criada com sucesso!');
      setTimeout(() => navigate(`/turma/${id}`), 2000);
    } catch (err) {
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-[#E8EFF7]" style={{ fontFamily: "'Syne',sans-serif" }}>Cadastrar Nova Aula</h1>
      </div>
